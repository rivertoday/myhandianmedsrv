...
public abstract class UmengNotification {
	...
	public String send() throws Exception {
        ...
        if (status == 200) {
            ...
            sendResult.append("发送成功!");
        } else {
            System.out.println("Failed to send the notification!");
            if(status == 500){
            	 sendResult.append("发送失败,状态码:"+status+"，错误解释-->内部错误；");
            }else if(status == 2026){
           	 	 sendResult.append("发送失败,状态码:"+status+"，错误解释-->1分钟内发送次数超出限制；");
            }else if(status == 2028){
              	 sendResult.append("发送失败,状态码:"+status+"，错误解释-->时间戳已过期；");
            }else
            {
            	 sendResult.append("发送失败,状态码:"+status+"；");
            }
        }
